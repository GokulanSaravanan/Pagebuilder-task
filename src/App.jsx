...
function App() {
    ...
    if (e.key === "Enter") {
      ...
      setJsonData((prevData) => ({
        ...
        blocks: prevData.blocks.map((block) => ({
          ...block,
          children: block.children.map((child) =>
            child.id === textId ? { ...child, content: newText } : child
          ),
        })),
      }));
    }
  ...
  return (
    <div className="App">
      <div className="nav">
        <div>
          <h1 style={{ color: "white" }}>Page Builder</h1>
        </div>
            ...
            <div key={block.id} className="page-block">
              ...
              >
                <div className="name-new-btns">
                  <h3 style={{ color: "white" }}>{block.id}</h3>
                  <button
                    className="btn"
                    onClick={() => handleAddImage(block.id)}
                  >
                    I
                  </button>
                  <button
                    className="btn"
                    onClick={() => handleAddLabel(block.id)}
                  >
                    T
                  </button>
                </div>
                <div className="children">
                  {block.children.map((child) => (
                    <p key={child.id} className="child">
                      {child.type}
                    </p>
                  ))}
                </div>
        ...
        <div className="page">
          {jsonData.blocks.map((block) => (
            <div
              ...
              onMouseMove={handleMouseMoveChild}
            ...
            >
              {block.children.map((child) => (
                ...
                >
                  {child.type === "text" && (
                    <div
                      onClick={() => handleEditText(block.id, child.id)}
                      onMouseDown={(e) =>
                        handleMouseDownChild(e, block.id, child.id)
                      }
                    >
                      {child.isEditing ? (
                        <input
                          ...
                          value={child.content}
                          onChange={(e) =>
                            handleChangeText(e, block.id, child.id)
                          }
                          onKeyDown={(e) =>
                            handleChangeText(e, block.id, child.id)
                          }
